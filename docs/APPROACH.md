...
| A host file listing or file read can be incomplete. | Account for every enumerated remote path as indexed or skipped with a reason, and label a truncated listing as incomplete. | Makes remote coverage limits actionable without falling back to a same-named local path. |
